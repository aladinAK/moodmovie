...
export function BookList({ subject }: BookListProps) {
  ...
  const fetchBooks = useCallback(async (bookSubject: string, index: number) => {
    setLoading(true)
    try {
      const orderOptions = ['relevance', 'newest']
      const orderBy = orderOptions[Math.floor(Math.random() * orderOptions.length)]
      const timestamp = Date.now()
      const response = await fetch(`/api/books?subject=${bookSubject}&startIndex=${index}&orderBy=${orderBy}&_=${timestamp}`)
      const data = await response.json()

      if (data.items && data.items.length > 0) {
        // Filtrer : image + titre + rating >= 3.5/5 (équivalent à 7/10 TMDB)
  ...
  useEffect(() => {
    ...
    setStartIndex(0)
    fetchBooks(subject, 0)
  }, [fetchBooks, subject])

  const handleRefresh = () => {
    const nextIndex = startIndex + 40
    setStartIndex(nextIndex)
    fetchBooks(currentSubject, nextIndex)
  }
